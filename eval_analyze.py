...
def analyze_and_save(args, eval_args, device, generative_model,
    ...
    stability_dict, rdkit_metrics = analyze_stability_for_molecules(
        molecules, dataset_info, bfn_schedule=args.bfn_schedule)
...
def main():
    ...
    parser.add_argument('--non_uniform_timestep', type=int, default=0,
                    help='whether use handcrafted non-uniform timestep in sampling')
    parser.add_argument('--temp_index', type=float, default=0.0,
                    help='temperatureindex in sampling')
    parser.add_argument('--prior_var', type=float, default=1.0,
                    help='prior_variance in sampling')
    
    parser.add_argument('--sample_steps', type=int, default=1000, help='Number of sampling steps')
    ...
    args.non_uniform_timestep = eval_args.non_uniform_timestep
    args.temp_index = eval_args.temp_index
    args.prior_var = eval_args.prior_var
    args.sample_steps = eval_args.sample_steps # may be different
