...
def get_model(args, device, dataset_info, dataloader_train, uni_diffusion=False, use_basis=False, decoupling=False, pretrain=False, finetune=False):
    ...
    if args.probabilistic_model == 'diffusion':
        vdm = EnVariationalDiffusion(
            ...
            str_schedule=args.str_schedule if "str_schedule" in args else False,
            sigma_min=args.sigma_min if "sigma_min" in args else 0.05,
            str_schedule_norm=args.str_schedule_norm if "str_schedule_norm" in args else False,
            bond_pred=args.bond_pred if 'bond_pred' in args else False,
            t_cont=args.t_cont if 't_cont' in args else False,
            prior_var=args.prior_var if 'prior_var' in args else 1.0,
            temp_index=args.temp_index if 'temp_index' in args else 0.0,
            non_uniform_timestep=args.non_uniform_timestep if 'non_uniform_timestep' in args else 0,
            )
