...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def __init__(
            ...
            relay_sampling=0, second_dynamics=None, sampling_threshold_t=10, atom_type_pred=True, bfn_schedule=False, device='cpu', atom_types=5, str_schedule=False, sigma_min=0.05, str_schedule_norm=False, 
            bond_pred=False, t_cont=False, prior_var=1, temp_index=0, non_uniform_timestep=0,
            **kwargs):
        super().__init__()
        ...
        self.str_schedule = str_schedule # for straight line schedule
        self.sigma_min = sigma_min
        self.str_schedule_norm = str_schedule_norm
        self.bond_pred = bond_pred
        self.non_uniform_timestep = non_uniform_timestep
        
        self.prior_var = prior_var
        self.temp_index = temp_index
        
        self.t_cont = t_cont # time continous
    ...
    def phi(self, x, t, node_mask, edge_mask, context, t2=None, mask_y=None):
        ...
        if self.relay_sampling == 0:
            if self.str_schedule_norm:
                x_in = x.clone()
                coord_scale = 1 / torch.sqrt(((1 - t)**2) * 3 + self.sigma_min ** 2)
                x_in[:, :, : 3] = x_in[:, :, : 3] * coord_scale.unsqueeze(1)
                net_out = self.dynamics._forward(t, x_in, node_mask, edge_mask, context, t2=t2, mask_y=mask_y)
            else:
    ...
    def sample_p_xh_given_z0(self, z0, node_mask, edge_mask, context, fix_noise=False):
        ...
        if self.str_schedule: 
            xh = z0
            # xh = z0 - self.sigma_min * net_out   #add one more step
        else:
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always, mask_indicator=None,
                     property_label=None, time_upperbond=-1, epoch=None, bond_info=None):
        ...
        if not self.str_schedule:
            ...
            sigma_min = torch.tensor(self.sigma_min)
            alpha_t = self.inflate_batch_array(1-t, x)
            
            # t can continous under the str schedule
            if self.t_cont:
                t = torch.from_numpy(np.random.uniform(low=0, high=1, size=(x.size(0), 1))).float().to(x.device)
                t = torch.clamp(t, min=0, max=0.999)
        ...
        if self.uni_diffusion:
            ...
            # load z_t, t, node_mask, edge_mask, network parameters
            # torch.save(self.dynamics.state_dict(), "phi_params.pth")
            #     # save z_t, t, node_mask, edge_mask
            # torch.save(z_t, "z_t.pth")
            # torch.save(t, "t.pth")
            # torch.save(node_mask, "node_mask.pth")
            # torch.save(edge_mask, "edge_mask.pth")
            # print('error bigger than 10')
            # load back
            # self.dynamics.load_state_dict(torch.load("atom_type_pred_k6_str_schedule_sigma_min_0.01_debug_clash/phi_params.pth"))
            # z_t = torch.load("atom_type_pred_k6_str_schedule_sigma_min_0.01_debug_clash/z_t.pth")
            # t = torch.load("atom_type_pred_k6_str_schedule_sigma_min_0.01_debug_clash/t.pth")
            # node_mask = torch.load("atom_type_pred_k6_str_schedule_sigma_min_0.01_debug_clash/node_mask.pth")
            # edge_mask = torch.load("atom_type_pred_k6_str_schedule_sigma_min_0.01_debug_clash/edge_mask.pth")
            
            # idx 25
            # z_t = z_t[25:26]
            # t = t[25:26]
            # node_mask = node_mask[25:26]
            # edge_mask = edge_mask[25:26]
            # gamma_t = gamma_t[25:26]
            # eps = eps[25:26]
        ...
        if mask_indicator is not None and mask_indicator:
            ...
            error = self.compute_error(net_out, gamma_t, eps)
            if error.mean().item() > np.mean(self.error_lst) * 5 and epoch > 1:
                print('current error: ', error.mean().item())
                print(f'error_lst: {self.error_lst}, mean error: {np.mean(self.error_lst)}')
                error = torch.zeros_like(error)
                loss_zero = True
            else:
                self.error_lst.append(error.mean().item())
                if len(self.error_lst) > 5:
                    self.error_lst.pop(0)
                loss_zero = False
            # if error.mean().item() > 2:
            #     error = torch.zeros_like(error)
            ...
            # if error.mean().item() > 10 and epoch > 1:
            #     # save self.phi parameters
            #     # torch.save(self.dynamics.state_dict(), "phi_params.pth")
            #     # # save z_t, t, node_mask, edge_mask
            #     # torch.save(z_t, "z_t.pth")
            #     # torch.save(t, "t.pth")
        ...
        if self.dynamics.mode == "PAT" or self.atom_type_pred:
            ...
            if loss_zero:
                atom_type_loss = torch.zeros_like(atom_type_loss)
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None, mask_indicator=None, expand_diff=False, property_label=None, bond_info=None, epoch=None):
        """
        ...
        if self.training:
            ...
            if expand_diff:
                ...
                loss, loss_dict = self.compute_loss(x, h, node_mask, edge_mask, context, t0_always=False, mask_indicator=mask_indicator, property_label=property_label, epoch=epoch, bond_info=bond_info)
        else:
    ...
    def sample_p_zs_given_zt(self, s, t, zt, node_mask, edge_mask, context, fix_noise=False, yt=None, ys=None, force_t_zero=False, force_t2_zero=False, pesudo_context=None):
        ...
        if self.dynamics.mode == "PAT" or self.atom_type_pred:
            ...
            if self.str_schedule:
                sigma_min = torch.tensor(self.sigma_min)
                if t[0] == 1:
                    mu = 2 * (zt - sigma_min * eps_t[:,:,0:3])
                else:
                    mu = ((1-s)/(1-t)).unsqueeze(1)*(zt - sigma_min * eps_t[:,:,0:3])
            else:
        ...
        if self.str_schedule:
            # index = 3 # temperature, default=0
            index = self.temp_index
            sigma = torch.sqrt(torch.tensor(2.0)) * sigma_min * (t[0].squeeze())**index
        else:
        ...
        if self.str_schedule and s[0] == 0:# add one more step
            zs = mu
        zs = torch.clamp(zs, min=-10, max=10) # add clamp in str
    ...
    @torch.no_grad()
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False, condition_generate_x=False, annel_l=False, pesudo_context=None, sample_steps=-1):
        """
        ...
        if fix_noise:
            ...
            if self.str_schedule:
                ...
                z = sigma_min * z * prior_var  # adjust prior variance, default *1
        ...
        #non_uniform_timestep setting
        if self.str_schedule and self.non_uniform_timestep:
            threshold=0.5 # 0<threshold<1
            steps_part1 = int(np.floor(sample_steps *1/3) + 1)
            steps_part2 = int(np.ceil(sample_steps *2/3) + 1)
            timetable_part1 = torch.linspace(0, threshold, steps_part1)
            timetable_part2 = torch.linspace(threshold, 1, steps_part2)[1:]
            timetable = torch.cat((timetable_part1, timetable_part2)).to(z.device)
            # print(timetable)
        ...
        for s in reversed(range(0, self.T)):
            if self.str_schedule and s==self.T-1: # add two more step
                continue
            #s=T-1,...0; t=T,...1
            if self.str_schedule and self.non_uniform_timestep:
                
                s_array = torch.full((n_samples, 1), fill_value=timetable[s], device=z.device)
                t_array = torch.full((n_samples, 1), fill_value=timetable[s+1], device=z.device)
            else:
    ...
    def sample_chain(self, n_samples, n_nodes, node_mask, edge_mask, context, keep_frames=None, annel_l=False):
        ...
        # Iteratively sample p(z_s | z_t) for t = 1, ..., T, with s = t - 1.
        for s in reversed(range(0, self.T)):
            if self.str_schedule and s==self.T-1: # add two more steps
                continue
